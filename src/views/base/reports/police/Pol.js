...
const CustomStyles1 = ({ rows, setRows, searchQuery, currentPage, pageSize, setCurrentPage }) => {
  ...
  const filteredRows = rows.filter(row =>
    ...
    row.sensetive.toLowerCase().includes(searchQuery.toLowerCase()) ||
    row.total.toLowerCase().includes(searchQuery.toLowerCase()) ||
    row.scan.toLowerCase().includes(searchQuery.toLowerCase()) ||
          ...
          <CTable bordered>
            <CTableHead>
              <CTableRow>
                ...
                <CTableHeaderCell>Sensitive Points</CTableHeaderCell>
                <CTableHeaderCell>Total QR Code</CTableHeaderCell>
                <CTableHeaderCell>Scan OR Code</CTableHeaderCell>
                <CTableHeaderCell>Total Scaned By Staff</CTableHeaderCell>
                ...
                <CTableRow key={row.id}>
                  ...
                  <CTableDataCell>
                    {row.isEditing ? (
                      <CFormInput
                        value={row.sensetive}
                        onChange={(e) => handleInputChange(e, row.id, 'sensetive')}
                      />
                    ) : (
                      row.sensetive
                    )}
                  ...
                  <CTableDataCell>
                    {row.isEditing ? (
                      <CFormInput
                        value={row.scan}
                        onChange={(e) => handleInputChange(e, row.id, 'scan')}
                      />
                    ) : (
                      row.scan
                    )}
                  ...
                  <CTableDataCell>
                    {row.isEditing ? (
                      <CFormInput
                        value={row.staff}
                        onChange={(e) => handleInputChange(e, row.id, 'staff')}
                      />
                    ) : (
                      row.staff
                    )}
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader className="d-flex justify-content-between align-items-center">
            <strong>Police Summary Report</strong>
