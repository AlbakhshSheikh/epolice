...
const CustomStyles = ({ formData, handleInputChange, handleSubmit }) => {
  ...
  return (
    ...
    >
      <CCol md={6}>
        <CFormLabel htmlFor="stationName">Master Police Station Name</CFormLabel>
        <CFormInput
          type="text"
          id="stationName"
          required
      ...
      <CCol md={6}>
        ...
        <CFormInput
          ...
          onChange={(e) => handleInputChange('Details', e.target.value)} // Corrected key to 'Details'
        />
          ...
          <CTableRow key={row.id}>
            ...
            <CTableDataCell>
              {row.isEditing ? (
                <CFormInput
                  value={row.Details}
                  onChange={(e) => handleInputChange(row.id, 'Details', e.target.value)} // Corrected key to 'Details'
                />
              ) : (
                row.Details
              )}
...
const Validation = () => {
  const [rows, setRows] = useState([
    { id: 1, Name: 'DYSP BALLAPUR', Details: 'Nagpur', status: 'Active', isEditing: false },
    { id: 2, Name: 'DYSP AKOT', Details: 'Akola', status: 'Active', isEditing: false },
  ])

  const [formData, setFormData] = useState({ Name: '', Details: '', status: '' })
  ...
  const handleSubmit = () => {
    ...
    setRows([...rows, newRow])
    setFormData({ Name: '', Details: '', status: '' }) // Reset form after submission
  }
  ...
  return (
    <CRow>
      <CCol xs={12} className="mb-4"> {/* Added margin-bottom to create space */}
        <CCard className="p-0"> {/* Added padding here */}
          <CCardHeader>
            <strong>Master Police Station</strong>
          </CCardHeader>
          <CCardBody>
      ...
      <CCol xs={12}>
        <CCard className="p-0 mb-4"> {/* Added padding here */}
          <CCardHeader>
